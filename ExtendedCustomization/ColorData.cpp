...
void ASColorData::GenerateColorMaterialData(ColorMaterialData* data, int num)
{
	if (this->CanGenerate())
	{
		auto part1 = CarPartDatabase::Instance->GetPartByNum(num > 3 ? Slot::BODY_PAINT_SWATCH : Slot::WHEEL_PAINT_SWATCH, ToInt(this->Index1 * 100.0f));
		auto part2 = CarPartDatabase::Instance->GetPartByNum(num > 3 ? Slot::BODY_PAINT_GROUP : Slot::WHEEL_PAINT_GROUP, ToInt(this->Index2 * 100.0f));
