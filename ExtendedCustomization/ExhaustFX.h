	...
	if (fxMarker->Hash == Hashes::EXHAUST_FX)
	{
		if (carRenderInfo->HasExhaustMerker())
		{
			if (strstr(model->Solid->Name, "EXHAUST_STYLE"))
			{
				...
				{
					auto marker = carRenderInfo->Extras->Exhaust.GetAdjustedMarker(fxMarker, carRenderInfo->Markers.LeftExhaust, false);
					CreateEmitter(marker, list, counter);
				}

				if (carRenderInfo->Markers.CenterExhaust)
				{
					auto marker = carRenderInfo->Extras->Exhaust.GetAdjustedMarker(fxMarker, carRenderInfo->Markers.CenterExhaust, false);
					CreateEmitter(marker, list, counter);
				}

				if (carRenderInfo->Markers.RightExhaust)
				{
					auto marker = carRenderInfo->Extras->Exhaust.GetAdjustedMarker(fxMarker, carRenderInfo->Markers.RightExhaust, true);
					CreateEmitter(marker, list, counter);
				}
			}
